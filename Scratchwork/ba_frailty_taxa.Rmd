...
```{r}
# Import data:
frailty$public_client_id <- factor(str_pad(as.character(frailty$public_client_id), width = 8, side = "left", pad = "0"))
frailty$sex <- factor(frailty$sex) #factorize sex
frailty$vendor_dashboard <- factor(frailty$vendor_dashboard)

print(otus)

names(taxonomy)[1] <- 'Name'
print(taxonomy)
```
...
names(frailty)[42:ncol(frailty)] <- meta$
...
heatmap.2(correlations, 
          ...
          cexRow = 0.35, # sets row text size
