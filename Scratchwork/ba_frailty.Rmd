...
#names(frailty) <- gsub("\\.$", "", names(frailty))
#taxonomy$Name <- gsub("\\.$", "", taxonomy$Name)
frailty
taxonomy
```
```{r}
...
```{r}
clr_taxa_limma <- read.csv(file = 'clrtaxa.csv')

# Find the minimum value in the count_matrix
min_val <- min(clr_taxa_limma[,2:ncol(clr_taxa_limma)])

# If the minimum value is less than 0, shift all values by adding the absolute value of min_val plus a small constant (e.g., 1)
if (min_val < 0) {
  clr_taxa_limma[,2:ncol(clr_taxa_limma)] <- clr_taxa_limma[,2:ncol(clr_taxa_limma)] + abs(min_val) + 1
}

clr_taxa_limma <- merge(frailty[,c(28:42)],clr_taxa_limma, by = "public_client_id")
countdf <- clr_taxa_limma[,c(1,6:9,13,16:ncol(clr_taxa_limma))]
rownames(countdf) <- clr_taxa_limma$public_client_id
countdf <- countdf[,c(2:6)]
countdf <- t(as.matrix(countdf))
...
#ignore this cell
clr_taxa_limma
as.data.frame(countdf)[4:nrow(countdf),]
for (i in range(1:length(split_string))) {
  split[i] <- paste0(split_string[[i]][5]," ", split_string[[i]][6])
  }
...
design <- model.matrix(~merge_fi + PC1 + PC2 + PC3 + PC4 + age + BMI_CALC + sex, frailty) # Covariates: sex, age, BMI, eGFR
...
```{r}
re_frailty <- topTable(fit, coef = 2, genelist = rownames(countdf), sort="p", number="none")  # Select the significant models by coefficient 2
re_frailty
```
...
```{r}
# Import data
meta <- read.csv(file = 'taxonomy.csv', sep = '\t')
split = c()
#Get results table for Constipation coefficient relative to High Normal BMF:
#names(clr_taxa_limma)[16:ncol(clr_taxa_limma)] 
split_string <- str_split(names(clr_taxa_limma)[16:ncol(clr_taxa_limma)], pattern = "\\.")
...
re_frailty <- topTable(fit, coef = 2, genelist = names(clr_taxa_limma)[16:ncol(clr_taxa_limma)], sort="p", number="none")  # Select the significant models by coefficient 2
indices <- match(rownames(re_frailty), meta$CHEMICAL_ID) # associate column of labs names with index of lab
re_frailty[1] <- names(frailty)[16:ncol(frailty)][indices] # associate the labs names with the labs indices
names(re_frailty)[1] <- 'dependent_vars'
...
select_frailty <- cbind(frailty[,1:35],frailty[,names(frailty) %in% sig_frailty$BIOCHEMICAL_NAME])
select_frailty
```
...
re_frailty <- topTable(fit, coef = 2, genelist = names(clr_taxa_limma)[16:ncol(clr_taxa_limma)], sort="p", number="none")  # Select the significant models by coefficient 2
indices <- match(rownames(re_frailty), meta$CHEMICAL_ID) # associate column of labs names with index of lab
re_frailty[1] <- names(frailty)[35:ncol(frailty)][indices] # associate the labs names with the labs indices
...
# Assuming 'frailty' is your data frame
frailty$public_client_id <- str_pad(frailty$public_client_id, width = 8, pad = "0")
df_matrix_filtered
clr_taxa <- read.csv('clrtaxa.csv')
clr_taxa$public_client_id <- str_pad(clr_taxa$public_client_id, width = 8, pad = "0")
#clr_taxa$public_client_id <- sub("^0+","",clr_taxa$public_client_id)
clr_taxa[match(rownames(df_matrix[,c(1:42)]),clr_taxa$public_client_id),]
...
```{r}
df_taxa <- df_matrix[, paste0("taxa_",taxa)]
